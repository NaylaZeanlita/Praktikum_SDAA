...
void badcharHeuristic(string x, int size, int badChar[NO_OF_CHARS])
{
...
Proyek *boyermooreproyek(Proyek *head_proyek, string x)
{
    int m = x.size();
    ...
    while (temp != nullptr)
    {
        ...
        while (s <= (n - m))
        {
            ...
            {
                s += max(1, j - badChar[(int)temp->nama[s + j]]);
            }
        }
        temp = temp->next;
    }
...
void menuPencarian()
{
    ...
    do
    {
        ...
        case 3:
        {
            ...
            cin.ignore();
            getline(cin, nama);
            Proyek *result = boyermooreproyek(head_proyek, nama);
            if (result)
                cout << "Proyek ditemukan: \n"
                     << setw(5) << "Kode" << setw(30) << "Nama" << setw(15) << "Status\n"
                     << setw(5) << result->kode
                     << setw(30) << result->nama
                     << setw(15) << result->status << endl;
            else
